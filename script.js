...
function getFleetRank(level = getEffectiveLevel()) {
  // Fleet Level is the ship size. Level 5 is the default ship.
  // Downtime rewards are described to players as Fleet Rank.
  // Production table row 5 = Fleet Rank 1, row 6 = Fleet Rank 2, and so on.
...
function getAllMaterials() {
  ...
  state.portsData.ports.forEach(port => {
    port.materials.forEach(material => names.add(material.name));
  });

  state.optionsData.options.forEach(option => {
    option.adventures.forEach(adventure => {
      adventure.materials.forEach(material => names.add(material.name));
    });
  });
...
function populateRitualSelect() {
  ...
  function addOptionToSelect(key, ritual) {
  ...
  function addOptionToGroup(group, key, ritual) {
    const option = document.createElement("option");
    option.value = key;
  ...
  noRitual.forEach(([key, ritual]) => addOptionToSelect(key, ritual));
  ...
  if (activitySpecific.length) {
    ...
    activitySpecific.forEach(([key, ritual]) => {
      addOptionToGroup(group, key, ritual);
    });
...
function filterPorts() {
  ...
  return state.portsData.ports.filter(port => {
    ...
    const matchesMaterial =
      state.material === "all" ||
      port.materials.some(material => {
        return material.name === state.material && getProductionAmount(material, effectiveLevel) > 0;
      });

    const haystack = [
      port.name,
      port.region,
      ...port.materials.map(material => material.name)
    ].join(" ").toLowerCase();
...
function filterAdventures() {
  ...
  return activity.adventures.filter(adventure => {
    const matchesMaterial =
      state.material === "all" ||
      adventure.materials.some(material => {
        return material.name === state.material && getProductionAmount(material, effectiveLevel) > 0;
      });
    ...
    const haystack = [
      activity.name,
      adventure.name,
      ...adventure.materials.map(material => material.name)
    ].join(" ").toLowerCase();
...
function renderEffectiveLevel() {
  ...
  const parts = [`Fleet Level ${state.baseLevel}`];
  ...
  els.effectiveLevelNote.textContent =
    `${activityLabel}: ${parts.join(" ")} = Effective Fleet Rank ${rank} (production row ${level})`;

  if (level < DEFAULT_FLEET_LEVEL) {
    els.effectiveLevelNote.textContent += " — using debuffed production rows.";
  }
  ...
  els.modifierBreakdown.innerHTML = `
    ...
    <div class="breakdown-row"><span>Fleet level</span><strong>Level ${state.baseLevel}</strong></div>
    ...
    ${extraRingsRow}
    <div class="breakdown-row breakdown-row--total"><span>Effective fleet rank</span><strong>${rank}</strong></div>
    <div class="breakdown-row"><span>Production table row</span><strong>${level}</strong></div>
  `;
}
...
function renderTradePorts() {
  ...
  ports.forEach(port => {
    ...
    if (port.id === state.selectedPortId) {
      card.classList.add("port-card--selected");
    }
    ...
    card.innerHTML = `
      <div class="port-card__header">
        ...
        <span class="level-pill">Rank ${getFleetRank(effectiveLevel)}</span>
      </div>
...
function renderOptionAdventures() {
  ...
  adventures.forEach(adventure => {
    ...
    if (adventure.id === state.selectedAdventureId) {
      card.classList.add("port-card--selected");
    }
...
function getCurrentSelection() {
  if (isTradeActivity()) {
    ...
    return port
      ? {
          name: port.name,
          subheading: port.region,
          materials: port.materials
        }
      : null;
  }
  ...
  return adventure
    ? {
        name: adventure.name,
        subheading: adventure.activityName,
        materials: adventure.materials
      }
    : null;
}
...
function renderSelectedOutput() {
  ...
  const produced = selection.materials
    .map(material => ({
      name: material.name,
      amount: getProductionAmount(material)
    }))
  ...
  if (!produced.length) {
    els.productionSummary.innerHTML = `<p class="empty-mini">No rewards at Effective Fleet Rank ${getFleetRank()}.</p>`;
    return;
  }
...
function openDetails(kind, id) {
  ...
  els.dialogContent.innerHTML = `
    ...
    <p class="subtitle">Production shown for Effective Fleet Rank ${getFleetRank(effectiveLevel)}.</p>
...
function copySummary() {
  ...
  const lines = [
    ...
    `Effective fleet rank: ${getFleetRank(effectiveLevel)} (production row ${effectiveLevel})`,
...
function loadState() {
  try {
    ...
    state.activityType =
      saved.activityType === TRADE_ACTIVITY ||
      state.optionsData.options.some(o => o.id === saved.activityType)
        ? saved.activityType
        : TRADE_ACTIVITY;
    ...
    const fallbackAdventure =
      activity?.adventures?.[0]?.id ||
      state.optionsData.options[0]?.adventures?.[0]?.id ||
      null;
...
function bindEvents() {
  ...
  els.chosenPort.addEventListener("change", event => {
    choosePort(event.target.value);
  });

  els.chosenOption.addEventListener("change", event => {
...
async function init() {
  ...
  try {
    const [portsData, optionsData, ritualsData] = await Promise.all([
      loadJson("ports.json"),
      loadJson("options.json"),
      loadJson("rituals.json")
    ]);
